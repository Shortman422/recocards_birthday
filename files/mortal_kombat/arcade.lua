...
local function round_over(winner)
 ...
 if match_point then
  S.phase="finish_intro"; S.finish_winner=winner; S.finish_ready=f+60; S.finish_timeout=0; S.finish_hit_at=0; S.finish_end=0; S.combo_step=0; S.combo_deadline=0; S.result="FINISH HIM!"; set_bgm("finish"); sfx("finish_him")
  if winner=="p" then
   S.e_y=0
...
local function update_fight()
 ...
 if S.phase=="finish_intro" then
  if f>=S.finish_ready then
   if S.finish_winner=="p" then S.phase="finish_ready"; S.finish_timeout=f+480; S.result=""
   else S.phase="scorp_finish_move"; S.result=""; set_p_state("finish_idle",0); set_e_state("walk",0) end
  end
  return
 end
 if S.phase=="finish_ready" then
  if S.finish_winner=="p" and S.e_state~="finish_idle" then set_e_state("finish_idle",0) end
  if S.finish_timeout>0 and f>=S.finish_timeout then
   S.finish_timeout=0
   land_finish(f)
   return
  end
